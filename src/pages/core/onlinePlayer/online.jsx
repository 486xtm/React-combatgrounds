...
export const OnlinePlayers = () => {
  const players = useSelector(({ online }) => online.onlinePlayers);
  const onlinePlayers = players.filter((i) => i);
  const navigate = useNavigate();
  return (
    <Layout currentActiveTab={"headquarters"}>
      <div className="flex-1 mx-5">
        <div className="text-[rgb(255,255,0)] text-[24px] font-[1000] my-5 underline">
          {onlinePlayers.length} Online Players
        </div>
        <div className="w-full min-h-[500px] h-auto text-[#FFFF00] ">
          ...
          {!onlinePlayers || onlinePlayers.length == 0 ? (
            ...
            onlinePlayers.map((user, index) => (
              <div className="w-full flex text-center" key={`online_${index}`}>
                <div className="w-[10%] border-[#FFFF00] border-[1px] leading-7">
                  {index + 1}
                </div>
                <a
